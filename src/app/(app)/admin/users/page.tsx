...
import { banUser, unbanUser, promoteUser, demoteUser, deleteUser, grantAdminByUsername, revokeAdminByUsername } from "../actions";
...
export default async function AdminUsers({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader><CardTitle className="text-sm font-medium">Grant / Revoke Admin</CardTitle></CardHeader>
        <CardContent className="flex gap-4">
          <form action={grantAdminByUsername} className="flex gap-2">
            <input name="username" placeholder="GitHub username" className="rounded border border-border bg-background px-3 py-1.5 text-sm" required />
            <Button type="submit" size="sm" variant="default">Grant Admin</Button>
          </form>
          <form action={revokeAdminByUsername} className="flex gap-2">
            <input name="username" placeholder="GitHub username" className="rounded border border-border bg-background px-3 py-1.5 text-sm" required />
            <Button type="submit" size="sm" variant="secondary">Revoke Admin</Button>
          </form>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm font-medium">Users ({allUsers.length})</CardTitle>
            <form method="get" className="flex gap-2">
              <input name="q" defaultValue={query} placeholder="Search username..." className="rounded border border-border bg-background px-3 py-1.5 text-sm" />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex gap-3 text-xs text-muted-foreground">
                      ...
                      <span>{Number(u.totalCommits).toLocaleString("en-US")} commits</span>
                      <span className="text-green-400">+{Number(u.totalAdditions).toLocaleString("en-US")}</span>
                ...
                <div className="flex gap-1">
                  {u.isBanned ? (
                    <form action={unbanUser}>
                      ...
                      <Button type="submit" size="sm" variant="secondary" className="text-xs">Unban</Button>
                    </form>
                  ) : (
                    <form action={banUser} className="flex gap-1">
                      <input type="hidden" name="userId" value={u.id} />
                      <input name="reason" placeholder="Reason" className="w-24 rounded border border-border bg-background px-2 py-1 text-xs" />
                      <Button type="submit" size="sm" variant="destructive" className="text-xs">Ban</Button>
                    </form>
                  )}
                  {u.isAdmin ? (
                    <form action={demoteUser}>
                      <input type="hidden" name="userId" value={u.id} />
                      <Button type="submit" size="sm" variant="secondary" className="text-xs">Demote</Button>
                    </form>
                  ) : (
                    <form action={promoteUser}>
                      <input type="hidden" name="userId" value={u.id} />
                      <Button type="submit" size="sm" variant="secondary" className="text-xs">Promote</Button>
                    </form>
                  )}
